...
// Shared layout constants — all sections use these for alignment
const MAX = "1080px";
const PX  = "32px";
const container: React.CSSProperties = { maxWidth: MAX, margin: "0 auto", padding: `0 ${PX}` };
  ...
  return (
    <main>
      ...
      <section style={{ position: "relative", padding: "120px 0 100px", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: 0, zIndex: 0, background: "radial-gradient(ellipse 70% 50% at 50% -5%,rgba(90,90,120,.1) 0%,transparent 70%)" }} />
        <div style={{
          position: "absolute", top: 0, bottom: 0, left: "50%", transform: "translateX(-50%)",
          width: `min(${MAX},100%)`, zIndex: 0,
        ...
        <div style={{ ...container, position: "relative", zIndex: 1 }}>
          <FadeIn>
            <div style={{ width: 80, height: 80, borderRadius: "50%", marginBottom: 24, background: "linear-gradient(135deg,#1a1a2e,#16213e)", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, color: "rgba(255,255,255,.3)", fontFamily: "'Inter',sans-serif", fontWeight: 700 }}>C</div>
            <div style={{ fontFamily: "'PingFang SC',-apple-system,sans-serif", fontSize: 40, fontWeight: 900, letterSpacing: "-.02em", lineHeight: 1.1, color: "#fff", marginBottom: 12, WebkitTextStroke: ".5px rgba(255,255,255,.4)" }}>CAIMAN</div>
            <div style={{ fontFamily: "'PingFang SC',-apple-system,sans-serif", fontSize: 20, fontWeight: 400, color: "rgba(255,255,255,.65)", marginBottom: 16 }}>
              设计师 · <strong style={{ fontWeight: 700, color: "rgba(255,255,255,.85)" }}>AI 原生</strong> · 独立创作者
            </div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {["UI / UX","品牌设计","AI 工具","前端开发","视觉叙事"].map(s => (
                <span key={s} style={{ border: "1px solid var(--border)", borderRadius: 999, padding: "5px 14px", fontSize: 12, color: "var(--muted)", fontFamily: "'Inter',sans-serif", letterSpacing: ".04em" }}>{s}</span>
              ))}
            </div>
          </FadeIn>
        </div>
      ...
      <section id="work" style={{ padding: "80px 0" }}>
        <div style={container}>
          <FadeIn><SectionHeader eyebrow="Work" title="精选项目" description="最新做的项目实验" /></FadeIn>
      ...
      {/* ── Intel ── */}
      <section id="intel" style={{ padding: "80px 0" }}>
        <div style={container}>
          <FadeIn><SectionHeader eyebrow="Intel" title="最近在看" description="精选链接、视频与阅读材料。" /></FadeIn>
          <FadeIn>
          ...
          <FadeIn>
            <div style={{ marginTop: 48, paddingTop: 36, borderTop: "1px solid var(--border)", marginBottom: 14, display: "flex", alignItems: "center", gap: 12 }}>
      ...
      {/* ── Design ── */}
      <section id="design" style={{ padding: "80px 0" }}>
        <div style={container}>
          <FadeIn><SectionHeader eyebrow="Design" title="攒劲设计" description="设计实验、参考资料与生产笔记。" /></FadeIn>
          <FadeIn>
            <HorizontalScroll>
      ...
      <section id="explore" style={{ padding: "80px 0" }}>
        <div style={container}>
          <FadeIn><SectionHeader eyebrow="Explore" title="其他探索" description="投资追踪、语言备考与旅行记录" /></FadeIn>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
            <FadeIn><ExploreCard eyebrow="Invest" title="股票理财"><InvestClient /></ExploreCard></FadeIn>
            <FadeIn delay={1}><ExploreCard eyebrow="English" title="语言学习"><CountdownSection /></ExploreCard></FadeIn>
          </div>
          <FadeIn delay={2}>
      ...
      {/* ── Footer ── */}
      <footer style={{ padding: `12px ${PX} 20px` }}>
        <div style={{ maxWidth: MAX, margin: "0 auto", height: 48, display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(8,8,8,.92)", backdropFilter: "blur(20px)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: `0 ${PX}` }}>
